...
void read_update_igm_density(confObj_t simParam, grid_t *thisGrid, int snap)
{
...
#ifdef VERBOSE
        if(thisRank == 0) printf(" Reading IGM density file %s\n", igm_density_file);
#endif
        ...
        {
                if(thisRank == 0) printf(" !!!NO DENSITY FILE EXISTS!!!\n assume a constant density field at mean density\n");
                simParam->dens_in_overdensity = 1;
                simParam->default_mean_density = 1;
        }
}
...
void read_update_igm_clump(confObj_t simParam, grid_t *thisGrid, int snap)
{
...
#ifdef VERBOSE
        if(thisRank == 0) printf(" Reading IGM clumping factor file %s\n", igm_clump_file);
#endif
        ...
        {
                if(thisRank == 0) printf(" !!!NO CLUMPING FACTOR FILE EXISTS!!!\n assume a clumping factor = 1\n");
        }
