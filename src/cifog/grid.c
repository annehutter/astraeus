...
void read_files_to_grid(grid_t *thisGrid, confObj_t thisInput)
{
    ...
    thisGrid->zreion = fftw_alloc_complex(alloc_local);
    thisGrid->photHI_zreion = fftw_alloc_complex(alloc_local);
    ...
    thisGrid->zreion = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*nbins*nbins*nbins);
    thisGrid->photHI_zreion = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*nbins*nbins*nbins);
    ...
    initialize_grid(thisGrid->zreion, nbins, local_n0, 0.);
    initialize_grid(thisGrid->photHI_zreion, nbins, local_n0, 0.);
...
void deallocate_grid(grid_t *thisGrid, confObj_t thisInput)
{
    ...
    if(thisGrid->zreion != NULL) fftw_free(thisGrid->zreion);
    if(thisGrid->photHI_zreion != NULL) fftw_free(thisGrid->photHI_zreion);
