...
#define getListFromIni(trgt, func, ini, keyName, sectionName, numListMembers)      \
    if (!func(ini, keyName, sectionName, numListMembers, trgt)) {              \
                fprintf(stderr,                                        \
                        "FATAL:  Could not get %s from section %s.\n", \
                        keyName, sectionName);                         \
                exit(EXIT_FAILURE);                                    \
...
extern bool
parse_ini_get_floatlist(parse_ini_t ini,
                        const char  *key_name,
                        const char  *section_name,
                        uint32_t    num_values,
                        float     **values);
